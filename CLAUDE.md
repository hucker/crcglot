...
- Full suite needs the per-target toolchains **on PATH**: `gcc`, `rustc`, `go`, `dotnet` (SDK), `javac`/`java`, `tsx` (Node), `zig`, `lua` (5.3+), `iverilog`, `ghdl`.  conftest does **not** hunt for install dirs (that list was a maintenance tax and it masked a stale shell instead of surfacing it); it only fixes the msys2-ahead-of-Git-Bash gcc ordering, which is a DLL-loading correctness issue rather than discovery.  A freshly installed tool is invisible to an already-open shell, so restart it (VSCode included, since VSCode-spawned shells inherit PATH at editor startup) — until then that target's tests SKIP, which is amber, not green.  CI runs the same full suite on Linux via `.github/workflows/exec.yml` (with a fail-on-skip gate).
- **Property-based tests (Hypothesis) belong on genuinely infinite axes only.**  Two exist: the `reverse()` never-confidently-wrong guarantee (`test_reverse.py`) and n-way chunking invariance (`test_stream.py`).  A countable axis (algorithms × variants × languages, or a closed record like `HexFormat`) gets an exhaustive parametrize instead — enumerate what can be enumerated.  Profiles live in `tests/conftest.py`: `dev` (random, the local default) and `ci` (derandomized, pinned by both workflows) so a release gate means the same thing on every run.  Pin any counterexample the `dev` profile finds as an `@example` so the `ci` profile carries it forever.
...
Claude Code usage reports produced by `/insights` live in `insights/`, at the repo root, and nowhere else.  Copy a new report out of `~/.claude/usage-data/` into `insights/` keeping the generated `report-YYYY-MM-DD-HHMMSS.html` filename, add its row to `insights/README.md`, and commit it.  Do not rename, edit, or regenerate a report after the fact: it is a snapshot of a window of sessions, and its value is that it says what it said on the day it ran.

**Only crcglot reports go here.**  A default `/insights` run samples sessions across every repo worked on in the window, so most raw reports are cross-project and belong nowhere near this repo.  Verify scope before filing: `~/.claude/usage-data/session-meta/*.json` carries a `project_path` per session, and every one must be this repo.  If any is not, re-run `/insights` scoped to crcglot instead of filing what you have.  `insights/README.md` has the check as a one-liner.

This repo is public, so read a report before committing it and confirm you are content to publish what it describes.

## EXAMPLES.md

- Auto-generated by `scripts/regenerate_examples.py`.  Never hand-edit; re-run the script and commit the regenerated file.
- **Three tours, each varying one axis**, not a cross-product: every language (same algorithm, no flags), every implementation variant (one language), every documentation style (one language per style, ownership derived as "first language by code that supports it").  Crossing the axes repeats itself; if you add a fourth axis, give it its own tour.
- One collapsible `<details>` block per cell.  Default collapsed; expandable on GitHub render.
- Quick links TOC at the top uses explicit `<a id="lang-{code}">` / `<a id="variant-{v}">` / `<a id="style-{code}-{style}">` anchors emitted by the script -- safe against `C#`/`C` anchor collisions.
- **Every block renders through `generate_files()`, the CLI's own front door**, and the `bash` line above it is the command that reproduces it.  Do not call `LanguageInfo.generator` directly here: it skips crcglot's file and class naming, which silently made the Java blocks unreproducible until the three-tour rewrite.  The `--small` flag is likewise required in the command whenever the block is bit-by-bit, because no flag means `--fast`.
- The script reads `LANGUAGES` and `VARIANT_ORDER`.  Adding a new language to `crcglot/targets.py` automatically picks it up on the next regeneration -- no separate maintenance of EXAMPLES.md required.
