...
- **C**: [bit-by-bit](#example-c-bitwise) · [table-driven](#example-c-table) · [slice-by-8](#example-c-slice8)
...
```zig
// crc32.zig -- generated by crcglot from reveng/crc32
// ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
...
pub fn crc32_update(state: u32, data: []const u8) u32 {
    var crc: u32 = state;
    for (data) |b| {
        crc ^= @as(u32, b);
        var i: u32 = 0;
...
pub fn crc32_self_test() bool {
    return crc32("123456789") == 0xCBF43926;
}
```
...
pub fn crc32_update(state: u32, data: []const u8) u32 {
    var crc: u32 = state;
    for (data) |b| {
        crc = crc32_table[@as(usize, @as(u8, @truncate(crc)) ^ b)] ^ (crc >> 8);
    }
    return crc;
}
...
const crc32_sliceTables = [8][256]u32{
    // T0
    [256]u32{
        0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3,
        0x0EDB8832, 0x79DCB8A4, 0xE0D5E91E, 0x97D2D988, 0x09B64C2B, 0x7EB17CBD, 0xE7B82D07, 0x90BF1D91,
...
pub fn crc32_update(state: u32, data: []const u8) u32 {
    ...
    while (d.len >= 8) {
        ...
        crc = crc32_sliceTables[7][@as(usize, @as(u8, @truncate(xored)))] ^ crc32_sliceTables[6][@as(usize, @as(u8, @truncate(xored >> 8)))]
            ^ crc32_sliceTables[5][@as(usize, @as(u8, @truncate(xored >> 16)))] ^ crc32_sliceTables[4][@as(usize, @as(u8, @truncate(xored >> 24)))]
            ^ crc32_sliceTables[3][@as(usize, @as(u8, @truncate(b47)))] ^ crc32_sliceTables[2][@as(usize, @as(u8, @truncate(b47 >> 8)))]
            ^ crc32_sliceTables[1][@as(usize, @as(u8, @truncate(b47 >> 16)))] ^ crc32_sliceTables[0][@as(usize, @as(u8, @truncate(b47 >> 24)))];
        d = d[8..];
    }
    for (d) |b| {
        crc = crc32_sliceTables[0][@as(usize, @as(u8, @truncate(crc)) ^ b)] ^ (crc >> 8);
    }
    return crc;
}
