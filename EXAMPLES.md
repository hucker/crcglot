...
// number of times over successive chunks of the message:
//
//     uint s = Crc32.crc32_init();
//     s = Crc32.crc32_update(s, chunk);  // over each chunk
//     uint crc = Crc32.crc32_finalize(s);
//
// One-shot: Crc32.crc32(data)
// Verify:   Crc32.crc32_self_test() returns true on success; run it once on your
//           toolchain to catch a compiler / endianness mismatch.
//
// Generated by crcglot from reveng/crc32 -- a verified reference implementation.
...
public static class Crc32
{
    ...
    //
    // Call init -> update (any number of times) -> finalize.
    public static uint crc32_update(uint state, byte[] data) {
        uint crc = state;
    ...
    //
    // Do not feed the finalized value back into update.
    public static uint crc32_finalize(uint state) {
    ...
    //
    // Run once on your target toolchain -- it is the cheapest way to catch a compiler / endianness / width mismatch before trusting the output.
    public static bool crc32_self_test() {
...
//
// Call init -> update (any number of times) -> finalize.
func crc32_update(state uint32, data []byte) uint32 {
    crc := state
    for _, b := range data {
        crc ^= uint32(b)
        for i := 0; i < 8; i++ {
...
//
// Do not feed the finalized value back into update.
func crc32_finalize(state uint32) uint32 {
    return state ^ 0xFFFFFFFF
}
...
// number of times over successive chunks of the message:
//
//     int s = CrcGlot.crc32_init();
//     s = CrcGlot.crc32_update(s, chunk);  // over each chunk
//     int crc = CrcGlot.crc32_finalize(s);
//
// One-shot: CrcGlot.crc32(data)
// Verify:   CrcGlot.crc32_self_test() returns true on success; run it once on your
//           toolchain to catch a compiler / endianness mismatch.
//
...
public final class CrcGlot {
    ...
    //
    // Call init -> update (any number of times) -> finalize.
    public static int crc32_update(int state, byte[] data) {
        int crc = state;
        for (byte b : data) {
            crc ^= (b & 0xFF);
            for (int i = 0; i < 8; i++) {
    ...
    //
    // width-32 results are a signed int bit pattern (negative when the high bit is set); call Integer.toUnsignedLong(x) for a 0..2^32-1 value.
    public static int crc32_finalize(int state) {
        return state ^ 0xFFFFFFFF;
    }
    ...
    //
    // Run once on your target toolchain -- it is the cheapest way to catch a compiler / endianness / width mismatch before trusting the output.
    public static boolean crc32_self_test() {
...
export function crc32_init(): number {
    return 0xFFFFFFFF;
}
...
//
// Do not feed the finalized value back into update.
export function crc32_finalize(state: number): number {
...
//
// Run once on your target toolchain -- it is the cheapest way to catch a compiler / endianness / width mismatch before trusting the output.
export function crc32_self_test(): boolean {
 ...
 *
 * One-shot: Crc32.crc32(data)
 * @note Verify with Crc32.crc32_self_test() (returns true on success); run it once on your toolchain to catch a compiler / endianness mismatch.
 *
 * @note Generated by crcglot from reveng/crc32 -- a verified reference implementation.
 */
...
// crc32_init return the initial CRC state to begin a computation.
//
// Returns the starting uint32 state.
func crc32_init() uint32 {
    return 0xFFFFFFFF
...
// crc32_update fold input into the running CRC state and return the new state.
//
// state: running uint32 state (from init or a prior update).
// data: the message bytes.
//
...
// crc32_finalize apply the final XOR to produce the CRC.
//
// state: accumulated uint32 state from update.
//
// Returns the finished CRC value.
...
// crc32_self_test self-test the implementation against the reveng catalogue.
//
// Returns true iff the CRC of "123456789" matches the embedded check value.
//
// Run once on your target toolchain -- it is the cheapest way to catch a compiler / endianness / width mismatch before trusting the output.
 ...
 *
 * One-shot: CrcGlot.crc32(data)
 * @note Verify with CrcGlot.crc32_self_test() (returns true on success); run it once on your toolchain to catch a compiler / endianness mismatch.
 *
