...
- **Lua**: [bit-by-bit](#example-lua-bitwise) · [table-driven](#example-lua-table)
...
- **Zig**: [bit-by-bit](#example-zig-bitwise) · [table-driven](#example-zig-table) · [slice-by-8](#example-zig-slice8)
...
// crc32 -- ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
// Parameters: width=32, poly=0x04C11DB7, init=0xFFFFFFFF, refin=true, refout=true, xorout=0xFFFFFFFF.
// check: Crc32.Compute(data) over "123456789" == 0xCBF43926
//
// Streaming (init -> update -> finalize); update may be called any
// number of times over successive chunks of the message:
//
//     uint s = Crc32.Init();
//     s = Crc32.Update(s, chunk);  // over each chunk
//     uint crc = Crc32.Finalize(s);
//
// One-shot: Crc32.Compute(data)
// Verify:   Crc32.SelfTest() returns true on success; run it once on your
//           toolchain to catch a compiler / endianness mismatch.
//
// Generated by crcglot from reveng/crc32 -- a verified reference implementation.
// Reproduce with crcglot:
...
public static class Crc32
{
    ...
    //
    // Call init -> update (any number of times) -> finalize.
    public static uint Update(uint state, byte[] data) {
        uint crc = state;
    ...
    //
    // Do not feed the finalized value back into update.
    public static uint Finalize(uint state) {
    ...
    public static bool SelfTest() {
        if (Compute(new byte[0]) != 0x00000000u) return false;
        if (Compute(new byte[] { 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39 }) != 0xCBF43926u) return false;
        {
            uint s = Init();
            for (int i = 0; i < 256; i++) {
                s = Update(s, new byte[] { (byte)i });
            }
            if (Finalize(s) != 0x29058C73u) return false;
        }
        {
            uint s = Init();
            ...
            if (Finalize(s) != 0x6D4552DBu) return false;
        }
        return true;
    }
}
...
-- crc32 -- ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
-- Parameters: width=32, poly=0x04C11DB7, init=0xFFFFFFFF, refin=true, refout=true, xorout=0xFFFFFFFF.
-- check: crc.crc32(data) over "123456789" == 0xCBF43926
--
-- Streaming (init -> update -> finalize); update may be called any
-- number of times over successive chunks of the message:
--
--     local s = crc.crc32_init()
--     s = crc.crc32_update(s, chunk)  -- over each chunk
--     local value = crc.crc32_finalize(s)
--
-- One-shot: crc.crc32(data)
-- Verify:   crc.crc32_self_test() returns true on success; run it once on your
--           toolchain to catch a compiler / endianness mismatch.
--
...
--
-- Call init -> update (any number of times) -> finalize.
function M.crc32_update(state, data)
    local crc = state
    for i = 1, #data do
        crc = crc ~ data:byte(i)
        for _ = 1, 8 do
...
--
-- Do not feed the finalized value back into update.
function M.crc32_finalize(state)
    return state ~ 0xFFFFFFFF
end
...
function M.crc32(data)
    return M.crc32_finalize(M.crc32_update(M.crc32_init(), data))
end
...
--
-- Run once on your target toolchain -- it is the cheapest way to catch a compiler / endianness / width mismatch before trusting the output.
function M.crc32_self_test()
    if M.crc32("") ~= 0x00000000 then return false end
    if M.crc32("123456789") ~= 0xCBF43926 then return false end
    local s = M.crc32_init()
    for i = 0, 255 do
        s = M.crc32_update(s, string.char(i))
    end
    if M.crc32_finalize(s) ~= 0x29058C73 then return false end
    s = M.crc32_init()
    for i = 0, 1023 do
        s = M.crc32_update(s, string.char((i * 167 + 13) & 0xFF))
    end
    if M.crc32_finalize(s) ~= 0x6D4552DB then return false end
    return true
end
...
```bash
crcglot lua crc32 --table
```

```lua
local crcglot_table_crc32 = {
...
function M.crc32_update(state, data)
    local crc = state
    for i = 1, #data do
        crc = crcglot_table_crc32[(crc ~ data:byte(i)) & 0xFF] ~ (crc >> 8)
    end
    return crc
end
...
//
//     var s = crc32Init();
//     s = crc32Update(s, chunk);  // over each chunk
//     const crc = crc32Finalize(s);
//
// One-shot: crc32(data)
// Verify:   crc32SelfTest() returns true on success; run it once on your
...
///
/// Do not feed the finalized value back into update.
pub fn crc32Finalize(state: u32) u32 {
    return state ^ 0xFFFFFFFF;
}
...
///
/// Run once on your target toolchain -- it is the cheapest way to catch a compiler / endianness / width mismatch before trusting the output.
pub fn crc32SelfTest() bool {
    if (crc32("") != 0x00000000) return false;
    if (crc32("123456789") != 0xCBF43926) return false;
    var s = crc32Init();
    var i: u32 = 0;
    while (i < 256) : (i += 1) {
        s = crc32Update(s, &[_]u8{@as(u8, @truncate(i))});
    }
    if (crc32Finalize(s) != 0x29058C73) return false;
    s = crc32Init();
    i = 0;
    while (i < 1024) : (i += 1) {
        s = crc32Update(s, &[_]u8{@as(u8, @truncate(i * 167 + 13))});
    }
    if (crc32Finalize(s) != 0x6D4552DB) return false;
    return true;
}
...
test "crc32 self-test" {
    if (!crc32SelfTest()) return error.SelfTestFailed;
}
```

</details>

<a id="example-zig-table"></a>
...
```bash
crcglot zig crc32 --table
```

```zig
const crcglot_table_crc32 = [256]u32{
    0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3,
    0x0EDB8832, 0x79DCB8A4, 0xE0D5E91E, 0x97D2D988, 0x09B64C2B, 0x7EB17CBD, 0xE7B82D07, 0x90BF1D91,
    0x1DB71064, 0x6AB020F2, 0xF3B97148, 0x84BE41DE, 0x1ADAD47D, 0x6DDDE4EB, 0xF4D4B551, 0x83D385C7,
    0x136C9856, 0x646BA8C0, 0xFD62F97A, 0x8A65C9EC, 0x14015C4F, 0x63066CD9, 0xFA0F3D63, 0x8D080DF5,
...
pub fn crc32Update(state: u32, data: []const u8) u32 {
    var crc: u32 = state;
    for (data) |b| {
        crc = crcglot_table_crc32[@as(usize, @as(u8, @truncate(crc)) ^ b)] ^ (crc >> 8);
    }
    return crc;
}
...
```bash
crcglot zig crc32 --slice8
```

```zig
const crcglot_slice_crc32 = [8][256]u32{
    // T0
    [256]u32{
        0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3,
        0x0EDB8832, 0x79DCB8A4, 0xE0D5E91E, 0x97D2D988, 0x09B64C2B, 0x7EB17CBD, 0xE7B82D07, 0x90BF1D91,
...
pub fn crc32Update(state: u32, data: []const u8) u32 {
    ...
    var d: []const u8 = data;
    while (d.len >= 8) {
        const b03: u32 = @as(u32, d[0]) | @as(u32, d[1]) << 8 | @as(u32, d[2]) << 16 | @as(u32, d[3]) << 24;
        const b47: u32 = @as(u32, d[4]) | @as(u32, d[5]) << 8 | @as(u32, d[6]) << 16 | @as(u32, d[7]) << 24;
        const xored: u32 = crc ^ b03;
        crc = crcglot_slice_crc32[7][@as(usize, @as(u8, @truncate(xored)))] ^ crcglot_slice_crc32[6][@as(usize, @as(u8, @truncate(xored >> 8)))]
            ^ crcglot_slice_crc32[5][@as(usize, @as(u8, @truncate(xored >> 16)))] ^ crcglot_slice_crc32[4][@as(usize, @as(u8, @truncate(xored >> 24)))]
            ^ crcglot_slice_crc32[3][@as(usize, @as(u8, @truncate(b47)))] ^ crcglot_slice_crc32[2][@as(usize, @as(u8, @truncate(b47 >> 8)))]
            ^ crcglot_slice_crc32[1][@as(usize, @as(u8, @truncate(b47 >> 16)))] ^ crcglot_slice_crc32[0][@as(usize, @as(u8, @truncate(b47 >> 24)))];
        d = d[8..];
    }
    for (d) |b| {
        crc = crcglot_slice_crc32[0][@as(usize, @as(u8, @truncate(crc)) ^ b)] ^ (crc >> 8);
    }
    return crc;
}
 ...
 *
 * One-shot: Crc32.Compute(data)
 * @note Verify with Crc32.SelfTest() (returns true on success); run it once on your toolchain to catch a compiler / endianness mismatch.
 *
 * @note Generated by crcglot from reveng/crc32 -- a verified reference implementation.
 * Reproduce with crcglot:
 *     version:   0.29.0
