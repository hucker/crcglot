...
```c
/* crc32.h -- generated by crcglot from reveng/crc32
 * ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
 */
#ifndef CRC32_H
#define CRC32_H
...
/* Streaming API: init -> update (any number of times) -> finalize. */
uint32_t crc32_init(void);
uint32_t crc32_update(uint32_t state, const uint8_t *data, size_t len);
uint32_t crc32_finalize(uint32_t state);

/* One-shot convenience: init + single update + finalize. */
uint32_t crc32(const uint8_t *data, size_t len);

/* Self-test: returns 0 if check value matches reveng catalogue, 1 otherwise. */
...
```c
/* crc32.c -- generated by crcglot from reveng/crc32
 * ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
 * check: crc("123456789") == 0xCBF43926
 *
 * Streaming: init -> update (any number of times) -> finalize.
 * One-shot:  call crc32(data, len).
 * Verify:    call crc32_self_test() (returns 0 on success).
 */
#include "crc32.h"
 ...
 * One-shot:  call crc32(data, len).
 * Verify:    call crc32_self_test() (returns 0 on success).
 * Variant:   slice-by-8 (8 tables, ~10x throughput vs plain table for large buffers).
 */
#include "crc32.h"
...
```csharp
// crc32.cs -- generated by crcglot from reveng/crc32
// ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
// check: Crc32.crc32({0x31..0x39}) == 0xCBF43926u
//
// Streaming: init -> update (any number of times) -> finalize.
// One-shot:  call Crc32.crc32(data).
// Verify:    call Crc32.crc32_self_test() (returns true on success).
...
```go
// crc32.go -- generated by crcglot from reveng/crc32
// ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
// check: crc32([]byte("123456789")) == 0xCBF43926
//
// Streaming: init -> update (any number of times) -> finalize.
// One-shot:  call crc32(data).
// Verify:    call crc32_self_test() (returns true on success).
...
```java
// crc32 -- generated by crcglot from reveng/crc32
// ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
// check: crc32("123456789") == 0xCBF43926
//
// Streaming: init -> update (any number of times) -> finalize.
// One-shot:  call CrcGlot.crc32(data).
//
// NOTE: width-32 results are returned as a signed int bit pattern; use Integer.toUnsignedLong(x) for a 0..2^32-1 value.
...
```python
def crc32_init() -> int:
    """Return the initial state for crc32 streaming CRC."""
    return 0xFFFFFFFF
...
def crc32_finalize(state: int) -> int:
    """Apply output reflection and xorout to finalize crc32."""
    return state ^ 0xFFFFFFFF
...
def crc32(data: bytes) -> int:
    ...
    check: crc(b'123456789') == 0xCBF43926
...
```systemverilog
// crc32.sv -- generated by crcglot from reveng/crc32
// ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
// check: crc("123456789") == 0xCBF43926
//
// Streaming: init -> update (one byte at a time) -> finalize.
// One-shot:  crc32(data) where ``data`` is a byte[] array.
// Verify:    call crc32_self_test in a testbench.
...
package crc32_pkg;
    ...
    // Run the canonical reveng check value; returns 1'b1 on success.
...
```vhdl
-- crc32.vhd -- generated by crcglot from reveng/crc32
-- ISO 3309, ITU-T V.42, Ethernet, PKZIP, PNG
-- check: crc("123456789") == 0xCBF43926
--
-- Streaming: init -> update (any number of times) -> finalize.
-- One-shot:  crc32(data).
-- Verify:    call crc32_self_test from a testbench's assert.
...
package crc32_pkg is
    -- Streaming triple.  ``state`` is a 32-bit std_logic_vector
    -- carrying the in-progress CRC value between updates.
    ...
    -- One-shot convenience: init + single update + finalize.
    -- Input ``data`` is a packed byte vector (length must be a multiple of 8).
    function crc32(data: std_logic_vector) return std_logic_vector;

    -- Self-test (returns true if algorithm matches reveng check).
