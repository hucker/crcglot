...
No trailer match.
Note: found a 32-byte trailing field matching no unkeyed digest; could be a MAC
(HMAC / CMAC -- keyed, unverifiable without the key) or an uncommon / truncated hash
```

Keyed MACs are undetectable by design — that's the third example's honest answer, and it's still useful: it ends the CRC hunt and names what the field probably is.  Confidence scales with `frames_agreed`: one frame is a hint, several corroborating frames are a finding.  Exit 0 on a match, 1 otherwise.
