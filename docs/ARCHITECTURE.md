...
That convention is the *mechanism* that keeps each axis first-class. The payoff
compounds two ways:

- **Cheap breadth.** Adding the Nth option touches one seam, not N call sites — a
  new language is picked up by `EXAMPLES.md`, the MCP `languages.json` resource,
