...
Every generated file header carries a `Reproduce with crcglot` provenance block of the resolved parameters (algorithm, target, variant, comment style, symbol, naming); it is always on, with no flag.  C additionally emits a linkable `const crcglot_provenance_t <symbol>_provenance` for runtime introspection of the CRC configuration, macro-guarded by `CRCGLOT_NO_PROVENANCE` and dropped by `--gc-sections` when unused.  The record carries only request-derived values (no tool version), so output stays a pure function of the request.  It is modelled by the public `ProvInfo` dataclass in `crcglot.comments` (built via `build_prov(...)`) and carried on `AlgoMeta.provenance`.
