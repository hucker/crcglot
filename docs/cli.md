...
| `symbol=NAME`         | Escape hatch: emit this exact identifier **verbatim**, bypassing `--naming`.  Verbatim means unchecked: the name must be legal in your target and not one of its keywords, or the generated file won't compile.  Single algorithm; not for Java.  Prefer `name=` for the usual "call it X" case. |
