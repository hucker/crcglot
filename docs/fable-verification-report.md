...
All seven findings were fixed on the branch that landed this report (`bug/fable-review-fixes`), the same day. Everything above is left as written at review time.

- **Finding 1 (CrcStream keyword constructor): fixed.** The constructor now builds a `Crc` from its raw integers, inheriting the width bound and the field ranges with identical messages (`src/crcglot/stream.py`); the docstring width list reads 1..64. Regression: `TestCrcStreamKeywordValidation` in `tests/test_stream.py`.
- **Finding 2 (CLI `--custom` widths): fixed.** The 8/16/32/64 pre-check is gone; the CLI delegates validation to `custom_algorithm`, so every width the generators support (1..64) now works from the command line and the message matches the Python API. The parameter table in `docs/cli.md` says 1..64. Regression: the width tests in `tests/test_cli.py`'s `TestCodegenCustom`.
- **Finding 3 (custom provenance overclaim): fixed.** A custom's header now reads "Generated by crcglot from custom parameters (not a reveng catalogue entry ...)", and its self-test doc reads "against its embedded check value" with a note stating the single check value is crcglot-computed, in every language and comment style (the doxygen and google styles carried their own copies of the origin line; all three now share one helper). A custom whose parameters match a catalogue entry keeps the independent-references self-test wording, which is true for it: parameter resolution hands it the four goldens. Regression: `TestCustomProvenanceScoping` in `tests/test_comments.py`.
- **Finding 4 (field ranges): fixed.** `Crc.__post_init__` range-checks `poly` / `init` / `xorout` against the width, so every door that speaks Crc (`custom_algorithm`, `CrcStream`, the CLI) rejects a negative or above-width value instead of masking it, echoing the field, the valid range, and the value. The raw engine functions keep their masking: they take bare integers below the validation boundary and must stay bit-identical to the C engine. Regression: `TestCrcFieldValidation` in `tests/test_catalogue.py`.
- **Finding 5 (exotic memoryviews): fixed.** `generic_crc`, `generic_crc_many`, and `CrcStream.update` normalize buffers at the boundary: a contiguous non-byte view is re-cast to bytes (zero-copy) so every engine hashes the same bytes, a non-contiguous view raises one friendly `ValueError` on every path, and a non-buffer type gets the same "a bytes-like object is required" TypeError whichever engine would have run. Regression: `TestBufferNormalization` in `tests/test_catalogue.py`.
- **Finding 6 (error leaks): fixed.** `reverse()` validates frame shape with the same rules and message as `reverse_packets`, plus a pointer to `reverse_packets()` for raw packet bytes; `generate_files` rejects a `Crc` / `AlgorithmInfo` passed as `algorithm` (pointing at `custom=`) and a wrong-typed `custom` (echoing the type). The pure-Python wrong-type leak is closed by the Finding 5 normalization. Regressions: `TestReverseFrameShapeValidation` in `tests/test_reverse.py` and the new `TestErrors` cases in `tests/test_generate_files.py`.
- **Finding 7 (cruft): fixed.** The four `test_crc_codegen_exec` citations point at the per-language test files; the README self-test table row is scoped; the `reverse` note reads singular for one frame; and `docs/cli.md`'s provenance sentence now mentions the version stamp (it said "no tool version", stale since the version landed in the block). EXAMPLES.md was regenerated; its only diff was the version stamp (0.25.0 to 0.25.1), confirming catalogue output is otherwise byte-unchanged by these fixes.

One deliberate behavior change rides along: parameter values the engines previously masked silently (negative or above-width `poly` / `init` / `xorout`) are now rejected at every validated door. The raw engine functions (`_generic_crc_python`, the C extension's `c_generic_crc`) are unchanged.
