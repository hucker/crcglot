...
| 6 | Segmentation | every split position of the check string and of a 33-byte message digests identically, plus curated boundary-hazard framings; n-way chunkings with mid-stream empty chunks searched over arbitrary messages | `tests/test_stream.py::TestSegmentation` on both engine backends (fast); `tests/test_stream.py::TestChunkingInvarianceProperty` (property-based, both backends, fast); `tests/test_independent_vectors.py::TestChunkingInvariance` (fast) |
...
Two of these categories are searched rather than sampled.  Where an axis is genuinely infinite -- the off-catalogue CRC parameter space in row 9, the chunkings of a message in row 6 -- the tests are property-based (Hypothesis): the invariant is stated once, the library searches for a counterexample, and any failure is shrunk to a minimal reproducing case before it is reported.  Found counterexamples are pinned inline as explicit examples so they run forever after.  This does not replace the enumerated axes; the catalogue x variant x language cross stays exhaustive, because a countable space should be covered completely rather than sampled.  Nor is it unbounded: CI pins a derandomized profile with a fixed example budget, so a green run means the same thing on every machine, and the search is a bounded one -- evidence, not a proof.
