...
Stdlib only.  No third-party deps -- matches crcglot's pure-stdlib
posture so the release tooling runs anywhere a plain Python does.
"""
...
def run_out(
    ...
    (e.g. ``ruff``/``ty`` exit 1 when they have diagnostics to report,
    which is the data we want to *count*, not crash on).
    """
...
def validate_version(version: str) -> None:
    ...
    crcglot stores the bare ``N.N.N`` in pyproject.toml and tags it
    ``vN.N.N`` -- so the scripts take the bare form and add the ``v`` for
    git operations.
    """
    if version.startswith("v"):
        die(f"version must not start with 'v' (got {version!r}). Use '0.8.0', not 'v0.8.0'.")
