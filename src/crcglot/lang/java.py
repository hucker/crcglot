...
def _self_test_java(fname: str, check: int, width: int) -> list[str]:
...
def generate_java(
    ...
    if algo is None:
        return None
    return generate_java_from_entry(name, algo, symbol=symbol, variant=variant)
...
def generate_java_from_entry(
    ...
    lines: list[str] = []
    lines.append(f"// {fname} -- generated by crcglot from reveng/{name}")
    lines.append(f"// {desc}")
    lines.append(f"// check: {fname}(\"123456789\") == {_java_hex(check, w)}")
    lines.append(f"//")
    lines.append(f"// Streaming: init -> update (any number of times) -> finalize.")
    lines.append(f"// One-shot:  call {_DEFAULT_CLASS}.{fname}(data).")
    lines.append(f"//")
    lines.append(
        f"// NOTE: width-32 results are returned as a signed int bit pattern; "
        f"use Integer.toUnsignedLong(x) for a 0..2^32-1 value."
        if w == 32 else f"//"
    )
    lines.append(f"")
    lines.append(f"public final class {_DEFAULT_CLASS} {{")
    ...
    # ----- self-test -----
    lines.extend(_self_test_java(fname, check, w))
