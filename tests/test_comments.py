...
def test_plain_valid_for_every_language(lang: str) -> None:
    """``plain`` resolves for every target (it is the universal style)."""
    # Act
...
class TestCustomProvenanceScoping:
    """Generated comments claim reveng origin / independent references only when true.

    Regression for Finding 3 of the Fable verification report
    (docs/verification/2026-07-02-fable.md): every custom generation used to stamp
    "Generated by crcglot from reveng/<symbol> -- a verified reference
    implementation" and headline the self-test as "against independent
    reference CRCs", neither of which is true for a non-catalogue custom
    (its check value is crcglot-computed).  docs/generated-code.md's
    embedded-self-test section states the custom limitation correctly; the
    generated files must agree with it.
    """

    _REVENG_LINE = "a verified reference implementation"
    _CUSTOM_LINE = "custom parameters (not a reveng catalogue entry"
    _INDEP_SUMMARY = "against independent reference CRCs"
    _CUSTOM_SUMMARY = "against its embedded check value"
    _CUSTOM_NOTE = "no independent reference"

    @staticmethod
    def _content(language: str, **kwargs) -> str:
        files = LANGUAGES[language].generate_files(**kwargs)
        return "\n".join(f.content for f in files)

    @pytest.mark.parametrize("language", sorted(LANGUAGES))
    def test_catalogue_output_keeps_the_reveng_line(self, language):
        # Act
        src = self._content(language, algorithm="crc32")

        # Assert
        assert self._REVENG_LINE in src, (
            f"{language}: catalogue output lost the reveng origin line"
        )
        assert self._INDEP_SUMMARY in src, (
            f"{language}: catalogue self-test summary must claim independent refs"
        )

    @pytest.mark.parametrize("language", sorted(LANGUAGES))
    def test_custom_output_is_scoped(self, language):
        # Arrange -- a genuine non-catalogue polynomial (no goldens exist).
        from crcglot.catalogue import custom_algorithm

        cust = custom_algorithm(width=16, poly=0x1337, init=0xABCD)

        # Act
        src = self._content(language, custom=cust)

        # Assert -- no reveng claim, no independence claim, scoped note present.
        assert "reveng/" not in src, (
            f"{language}: custom output claims a reveng origin"
        )
        assert self._REVENG_LINE not in src, (
            f"{language}: custom output claims a verified reference"
        )
        assert self._CUSTOM_LINE in src, (
            f"{language}: custom output lacks the scoped origin line"
        )
        assert self._CUSTOM_SUMMARY in src, (
            f"{language}: custom self-test summary is not scoped"
        )
        assert self._CUSTOM_NOTE in src, (
            f"{language}: custom self-test note must state no independent reference"
        )

    def test_goldens_matching_custom_keeps_independent_wording(self):
        """A custom whose parameters coincide with a catalogue entry picks up
        the four independent goldens (parameter resolution), so its self-test
        may keep the independent-references wording; its origin line still
        reads as custom."""
        # Arrange -- crc5-usb's exact parameters through the custom door.
        from crcglot.catalogue import custom_algorithm

        cust = custom_algorithm(
            width=5, poly=0x05, init=0x1F, refin=True, refout=True, xorout=0x1F
        )

        # Act
        src = self._content("c", custom=cust)

        # Assert
        assert self._CUSTOM_LINE in src, (
            "params-matching custom must still read as custom in its origin line"
        )
        assert self._INDEP_SUMMARY in src, (
            "params-matching custom carries the catalogue goldens, so the "
            "independent-references summary is true and must stay"
        )

    @pytest.mark.parametrize(
        ("language", "style"),
        [("c", "doxygen"), ("python", "google")],
        ids=["c-doxygen", "python-google"],
    )
    def test_style_specific_headers_scope_the_custom_line_too(self, language, style):
        """doxygen and google build their own headers (separate copies of the
        origin line), so the scoping must hold there as well."""
        # Arrange
        from crcglot.catalogue import custom_algorithm

        cust = custom_algorithm(width=16, poly=0x1337)

        # Act
        src = self._content(language, custom=cust, comment_style=style)

        # Assert
        assert "reveng/" not in src, (
            f"{language}/{style}: custom output claims a reveng origin"
        )
        assert self._CUSTOM_LINE in src, (
            f"{language}/{style}: scoped origin line missing"
        )
